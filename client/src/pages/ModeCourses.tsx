...
function ModeCoursesPage({ mode, courses }: { mode: "start" | "ninja"; courses: Course[] }) {
  ...
  return (
    <main className={`mode-courses-page mode-courses-page--${mode}`}>
      ...
      <section className="mode-courses-intro"><p>{start ? (language === "en" ? `${courses.length} routes to begin a new skill with clarity and put it into practice.` : `${courses.length} rutas para empezar una nueva skill con claridad y llevarla a la práctica.`) : (language === "en" ? `${courses.length} routes to go deeper, practise and master a skill with better judgment.` : `${courses.length} rutas para profundizar, practicar y dominar una skill con más criterio.`)}</p><span>{String(courses.length).padStart(2, "0")} {language === "en" ? "selected courses" : "cursos seleccionados"}</span></section>
