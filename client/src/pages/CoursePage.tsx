  ...
  return (
    <main className={`course-page course-page--${course.mode === "Modo 2" ? "ninja" : "start"}`}>
      ...
      <section className="course-hero">
        ...
        <div className="course-hero-copy"><p className="mono-label">{course.mode} / {course.modeLabel}</p><p className="course-detail-category">{course.category}</p><h1>{course.title}</h1><p>{detail.eyebrow}</p><a href={course.officialUrl} target="_blank" rel="noreferrer" className="detail-cta">Más de este skill <ArrowUpRight size={17} /></a></div>
        <span className="course-hero-number">{course.number}</span>
      </section>
      <section className="course-story"><div><p className="mono-label">/ La propuesta</p><h2>Una ruta<br /><em>concreta.</em></h2></div><div><p className="course-story-lede">{course.longDescription}</p><p>{course.description}</p></div></section>
